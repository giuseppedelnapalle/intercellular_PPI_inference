...
"""
Created in Jun 2020
@title: supplementary functions for regularized product PPI inference algorithm
@description: 
...
# SECTION_4
# functions for compute_reg_product, compute_pvalue, significant_reg_product (method 3_5)
...
# main function
def compute_reg_product(arr_geps: np.array,
                        arr_interaction_idx: np.array,
                        arr_sample_pair_idx: np.array,
                        mu: float) -> np.array:
    """
    calculate the regularized product statistic for each interaction, sample, cell_type_pair combination
    regularized product defined as sqrt(l*r) / (mu+sqrt(l*r)), 
    where l, r denote expresson of ligand and receptor, mu denotes mean of all elements in expression matrix
    """
...
def reg_product_shuffled(arr_geps: np.array,
                         arr_interaction_idx: np.array,
                         mu: float,
                         cell_type: list,
                         sample_type: pd.DataFrame,
                         ...
                         seed_num: int = 123) -> np.array:
    """
    compute regularized product statistic on shuffled data of control group.
    ...
    """
    print('If argument exc_cell_type_ctrl provided, null distribution of reg_product estimated from control group, \
...
def _compute_pvalue_itparam(arr_stat: np.array, 
                            ...
                            iterations: int,
                            param: list  # list containing index for row & col of arr_reg_product
                            ) -> float:
    """
    compute p-values for regularized product based on estimated null distribution with multiprocessing framework
...
def _compute_pvalue_itparam_2(arr_stat: np.array, 
    ...
    """
    compute p-values for regularized product for arr_stat with NaN value
    """
    row, col = param[0], param[1]
...
def _sig_reg_prodct_itparam(arr_stat: np.array,
                            arr_pvalue: np.array,
                            alpha: float,
                            value_thresh: float,
                            row_index: int) -> np.array:
    """
    compute significant regularized product with multiprocessing framework
    If p.value < alpha, the value will be the reg_product. Alternatively, the value is set to value_thresh.
    """
