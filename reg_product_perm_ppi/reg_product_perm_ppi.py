...
"""
Created in Jun 2020
@title: infering intercellular PPI based on regularized product score
@reference: SingleCellSignalR
@description:
    algorithm for infering intercellular PPI [optimised version].
    regularized product of ligand and receptor expression in each cell type as statistic,
    permutation test used to test significicance of enrichment of PPI pairs.
    required functions imported from rpp_ppi_helpers module
    version 1.2 (compatible with rpp_ppi_helpers V1.2)
    $input 
    ...
    $output 
        1. data frame of regularized product of interactions between two proteins 
           from specified cell type pairs (pd.dataframe)
        2. data frame of probabilty associated with regularized product
        3. data frame of significant regularized product of interactions
@author: nikola
"""
...
import rpp_ppi_helpers as hp


# define class for the algorithm
# the RegProductPermPPI object is created from the merged gene expression profile (GEP) matrix derived from CIBERSORTx.
...
class RegProductPermPPI:
    ...
    def __init__(self, 
        ...
        """
        create a new RegProductPermPPI instance
        ...
        if test:
            ...
            print('Attributes of class RegProductPermPPI: geps, sample_id, cell_type, \
    ...
    # method 4
    def compute_reg_product(self) -> pd.DataFrame:
        """
        compute regularized_product_statistics measuring interaction potential from interacting cell types
        """
        # scaling factor (mean of all values in geps)
        mu = self.geps.to_numpy().sum() / (self.geps.shape[0]*self.geps.shape[1])
        ...
        print('Computing regularized product statistics for observed values.')
        
        arr_stat = hp.compute_reg_product(arr_geps, arr_inter_idx, arr_sp_idx, mu)
        ...
        print('New attributes: mu, df_stat.')
    ...
    def compute_pvalue(self, 
        ...
        arr_stat_shuf = hp.reg_product_shuffled(arr_geps, arr_inter_idx, self.mu, self.cell_type, 
    ...
    # method_6
    def significant_reg_product(self, 
                                alpha: float = .05,
                                iterations: int = 2000, 
                                default_threads: int = 8,
                                value_thresh: float = .01,
                                seed_num: int = 123):  # pd.dataframe added as attribute
        """
        compute significant regularized_product_statistic
        If p.value < alpha, the value will be the reg_product. Otherwise, the value is set to value_thresh.
        """
        # define required parameters
        arr_stat = self.df_stat.to_numpy(dtype=np.float64)
        arr_p = self.df_pvalue.to_numpy(dtype=np.float64)
        
        sig_reg_prodct_mp = partial(hp._sig_reg_prodct_itparam,
                                    arr_stat, arr_p,
                                    alpha, value_thresh)
        
        print('Computing significant regularized product statistics. Please be patient.')
        
        with Pool(processes = default_threads) as pool:
            sig_stat = pool.map(sig_reg_prodct_mp, range(arr_stat.shape[0]))  # list of np.array
